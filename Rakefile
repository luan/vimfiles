...
namespace :plugins do
  desc 'Compile stuff, like Command-T. UNIX only task'
  task :compile do
    print 'Compiling: Command-T... '
    if `cd ~/.vim/bundle/Command-T/ruby/command-t/ && /usr/bin/ruby extconf.rb && make`
      puts 'Done'
    end
  end
...
def parse_plugins_from_vimrc
  plugins = []
  File.new(VUNDLE_FILE).each do |line|
    if line =~ /^Bundle\s+["'](.+)["']/
